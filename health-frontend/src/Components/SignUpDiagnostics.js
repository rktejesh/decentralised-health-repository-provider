...
function SignUpHospital() {
  ...
  const requestingOtp = async () => {
    ...
    console.log(eth_id);
    await axiosPrivate
      .post("https://health-backend-1aeu.onrender.com/api/user/login", {
        mobile: ph_no,
        eth_id: eth_id,
      })
      .then((res) => {
  ...
  const Data = async (event) => {
    ...
    await axiosPrivate
      .post("https://health-backend-1aeu.onrender.com/api/user/verify-mobile", {
        eth_id: eth_id,
        mobile: ph_no,
        code: password,
      })
  ...
  const Loginto = async () => {
    ...
    await axiosPrivate
      .post("https://health-backend-1aeu.onrender.com/api/user/register", {
        eth_id: eth_id,
        email: email,
        name: Username,
        abha_id: DiagnosticsId
      })
      .then((res) => {
        if (res) {
          const accessToken = res.data.accessToken ;
          setAuth({Username,DiagnosticsId,email,accessToken});
          navigate("/Diagnostics/Home");
        }
      })
      .catch((err) => {
        setotpSent("Invalid OTP");
  ...
  return (
    <form action="#" className="sign-up-form">
      ...
      <div className="input-field">
        ...
        <input
          type="text"
          placeholder="Enter your Diagnostics ID"
          value={DiagnosticsId}
          onChange={(e) => {
