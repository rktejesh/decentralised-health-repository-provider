import { User, UserRecord, Token, HospitalRecord } from '../../../models/index.js';
...
export default async (req, res) => {
  ...
  const hospitalRecord = await HospitalRecord.find().catch((err) => {
    return res.status(500).json(errorHelper("00031", req, err.message));
  });

  if (!hospitalRecord) return res.status(400).json(errorHelper("00042", req));
