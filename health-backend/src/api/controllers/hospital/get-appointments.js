...
export default async (req, res) => {
  ...
  const appointmentRecord = await Appointment.find({ hospitalId: hospitalRecord.registrationId }).catch((err) => {
    return res.status(500).json(errorHelper("00031", req, err.message));
  });

  const doctorRecord = await DoctorRecord.find({ hospitalId: hospitalRecord.registrationId }).catch((err) => {
    return res.status(500).json(errorHelper("00031", req, err.message));
  });
