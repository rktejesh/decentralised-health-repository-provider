...
const { lookup } = geoip;
import registerDoctorLedger from '../contracts/registerDoctorLedger.js';
...
//   }
  ...
  const userexists = await DoctorRecord.findOne({ eth_id: req.body.eth_id })
    .catch((err) => {
      return res.status(500).json(errorHelper('00031', req, err.message));
    });
  ...
  req.body.doctorId = req.body.eth_id;
  req.body.address = "";
  req.body.aadhaar = "";
  req.body.DOB = "";
  ...
  await registerDoctorLedger(req);
  ...
  doctorRecord = await doctorRecord.save().catch((err) => {
